...
# Fallbacks from env/hardcoded to ensure run without .env
BOT_TOKEN = os.getenv("BOT_TOKEN", "8186857247:AAHdKaEOvVnubNwdEbbVpWTgUjXcgwGyNMc")
CRYPTO_PAY_TOKEN = os.getenv("CRYPTO_PAY_TOKEN", "480047:AAVg8Pwgbpqyk89R5CtAXLtLEObDdEKAaua")
try:
	ADMIN_ID = int(os.getenv("ADMIN_ID", "8355165232"))
except Exception:
	ADMIN_ID = 8355165232
...
async def help_cmd(message: Message):
	text = (
		...
		f"- Тарифы: 1м — ${PRICES[1]}, 3м — ${PRICES[3]}, 6м — ${PRICES[6]}, 12м — ${PRICES[12]}.\n"
		"- Управление: раздел ‘🧾 Мои аренды’ — просмотр активных аренд и продление.\n"
		"- Оплата: безопасные платежи в USDT через Crypto Pay.\n\n"
		"Порядок действий:\n"
		"1) Откройте раздел ‘📱 Номера’ и выберите свободный номер.\n"
		"2) Укажите срок аренды.\n"
		"3) Оплатите счёт (USDT) по сгенерированной ссылке.\n"
		"4) Нажмите ‘Я оплатил’ — бот автоматически подтвердит оплату и активирует аренду.\n\n"
...
async def paid_check(callback: CallbackQuery):
	...
	if not p:
		...
		return
	invoice = await crypto_client.get_invoice(p.get("invoice_id"))
	if not invoice:
